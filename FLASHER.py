...
from random import *
# Importing some Windows libraries. To use then type pip install pywin32 in console
...
for i in range(0, 100):
    ...
    SelectObject(desk, brush) # Choose that we're drawing with our brus
    PatBlt(desk, randrange(x), randrange,(y), randrange (x), randrange(y), PATINVERT) # Makes a lot of stuff :)
    DeleteObject(brush) # Free up memory. Pretty necessary to avoid crashes etc.
